import re
from layout_jyh.utils import find_useful_nodes, cal_expression, extract_expression
...
def find_array_in_node(node, constant_dict):
    array_list = []
    if node.get("nodeType", "") == "VariableDeclaration":
        var_name = node.get("name", "")
        if var_name != "":
            if node.get("typeName", {}):
                if node["typeName"].get("nodeType", "") == "ArrayTypeName":
                    ...
                    array_length_dict = extract_expression(array_length_dict)
                    if not array_length_dict:
                        return array_list
                    if array_length_dict.get("value", ""):
                        array_list.append(
                            {array_name: int(array_length_dict.get("value", ""))}
                        )
                    elif array_length_dict.get("name", ""):
                        array_list.append(
                            {
                                array_name: constant_dict[
                                    array_length_dict.get("name", "")
                                ]
                            }
                        )
                    else:
